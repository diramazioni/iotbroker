...
vs_name = ["Arganda","Ghilardino","Fabbrico","Cadriano","CAAB",    "OrtoBotanico","Ozzano"]
...
vs_area = ["6991ac61-8db8-4a32-8fef-c462e2369055","001", "002", "003","004", "005", "006"  ]
...
def retrieve_daily(i):
    keys  =  ['temp','day','min','max','humidity','pressure','wind_speed','wind_deg','rain']
    vars  =  ["tc", "tc_avg", "tc_min", "tc_max","humidity","pressure","wind_speed","wind_deg","rain"]
    ...
    rp = prefix+"/onecall?"+vs_loc+"&exclude=current,minutely,hourly,alerts&appid="+appid
    #
    print(rp)
    response = urlopen(rp)
...
def retrieve_hourly(i):
    ...
    rp = prefix+"/onecall?"+vs_loc+"&exclude=current,minutely,daily,alerts&appid="+appid
    # 
    print(rp)
    response = urlopen(rp)
    html_bytes = response.read()
...
# mi connetto a 'MQTTS WeLASER
    if not mqtts_connect(mqtts_client,MQTTS_USERNAME,MQTTS_PASSWORD, MQTTS_BROKER, MQTTS_PORT):
        return False
