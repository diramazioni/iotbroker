...
class MessageLogger:
    def __init__(self):
        ...
        self.mqtts_client.username_pw_set(
            self.MQTTS_USERNAME, password=self.MQTTS_PASSWORD
        )
    ...
    def MQTTS_connect(self, MQTTS_username, MQTTS_password, broker_endpoint, port):
        ...
        attempts = 0
        while (
            not self.mqtts_client.is_connected() and attempts < 5
        ):  # Wait for connection
            ...
            if interactive:
                ...
                while in_ not in ["x", "X"]:
                    ...
                    if in_ in ["a", "A"]:
                        ...
                        ptopic = (
                            f"{self.FIWARE}{self.ENTITY}device:WeatherStation_v8/attrs"
                        )
